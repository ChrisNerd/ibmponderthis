...
object ponder {
 ...
 val sol1 = (Set(Set(2, 6), Set(1, 6), Set(3, 7), Set(1, 5), Set(0, 5), Set(2, 7), Set(4, 7), Set(3, 6), Set(5, 7), Set(1, 4), Set(0, 3), Set(2, 5)),5)
 val sol2 = (Set(Set(2, 6), Set(1, 6), Set(3, 7), Set(1, 5), Set(0, 5), Set(2, 7), Set(4, 7), Set(3, 6), Set(5, 7), Set(1, 4), Set(0, 3), Set(2, 5)),7)

 def printAdjacencyMatrix(grid: Set[Set[Int]]) =
      (0 to 7).map { y =>
        println((0 to 7).map { x => if (grid.contains(Set(x, y))) "1" else "0" }.mkString)}

 val sol10 = (Set(Set(2, 6), Set(1, 6), Set(3, 7), Set(1, 0), Set(5, 0), Set(2, 7), Set(4, 7), Set(3, 6), Set(0, 7), Set(1, 4), Set(5, 3), Set(2, 0)),0)
 val sol20 = (Set(Set(2, 6), Set(1, 6), Set(3, 0), Set(1, 5), Set(7, 5), Set(2, 0), Set(4, 0), Set(3, 6), Set(5, 0), Set(1, 4), Set(7, 3), Set(2, 5)),0)
...
def time[R](block: => R): R = {
  ...
  def createMarkovMatrixFromPairs( setOfPairs: Set[Set[Int]], patientZero: Int ): Vector[Vector[Double]] = {
   ...
   val allStates = ((0 to n).toSet - patientZero).subsets.toList  // This is 1 to n, excluding 0. Person 0 is patient 0, so to speak.
//   println("AllStates should be 2^(n-1) " + allStates.size)
   allStates.toVector.map{ infectedSet =>
   /* val destinationStatesSet = allStates.filter( dest => (infectedSet -- dest).isEmpty)
  // println("infected Set " + infectedSet)
    ...
    */
    ...
    val transitionAmounts = neighboursOfInfectedflattend.subsets.map( newlyInfectedSet => (newlyInfectedSet ++ infectedSet,
    ...
    allStates.map( d => transitionAmounts.getOrElse(d, 0.0)).toVector
    /*
    ...
    // Need to
    allStates.map( d => if (d == infectedSet) probStay else if (d.isEmpty) 0.0 else mMultipleStateChanges.getOrElse(d, m1.getOrElse(d.head, 0.0) ))
    */
   }
  }
 ...
 def updateVector1Day( M: List[List[Boolean]]) = (v: List[Double]) => // List[Double] =
  ...
  def after10Days( M: List[List[Boolean]] ) : List[Double] ={
   ...
   def multiplyRowByColumn( r : List[Double], c : Vector[Double]): Double =
...
*/
   def multiplyRowByMatrix2 ( y: Vector[Vector[Double]])(r: List[Double]): List[Double] =
     r.zipWithIndex.map{ case (relement, rindex) => multiplyRowByColumn(r, y.map(yindex => yindex(rindex)))}
     

   def matrixMultiply(x: List[List[Double]], y: List[List[Double]]): List[List[Double]] =
     for (xrow <- x)
      yield List(0.0)//multiplyRowByMatrix(xrow, y)

 def after19Days(M: Vector[Vector[Double]]): List[Double] = {
    val v0 = 1.0 :: List.fill(M.size-1)(0.0)
    val update19 = Function.chain(List.fill(19)(multiplyRowByMatrix2(M)_ ))
    update19(v0)
  }
  
def mult[A](a: Vector[Vector[A]], b: Vector[Vector[A]])(implicit n: Numeric[A]): Vector[Vector[A]] = {
  import n._
  for (row <- a)
...
*/
  ...
  /*
   ...
   A10 = A8 *A2
   */
  def matrixToPower10(m: Vector[Vector[Double]]): Vector[Vector[Double]] = {
 	 val A2 = mult(m, m)
	 val A4 = mult(A2, A2)
	 val A8 = mult(A4, A4)
         mult(A8, A2)
  ...
  // Oh my god!!! We do not need to do expensive matrix multiplications after all!!!
  // I can't believe I wasted the cycles.
  // M*M*M*M*M...19 times * InitialState
  // Just do them from right to left!!!
  // Matrix * vector = vector.
  // Repeat 19 times.
  // Matrix * vector is just n^2 operations.
...
//val A2 = matrixMultiply(m, m)
 ...
 printAdjacencyMatrix(sol1._1)
 println(matrixToPower19(createMarkovMatrixFromPairs(sol1._1,sol1._2)).head.last)
 printAdjacencyMatrix(sol2._1)
 println(matrixToPower19(createMarkovMatrixFromPairs(sol2._1,sol2._2)).head.last)
 printAdjacencyMatrix(sol10._1)
 println(matrixToPower19(createMarkovMatrixFromPairs(sol10._1,sol10._2)).head.last)
 printAdjacencyMatrix(sol20._1)
 println(matrixToPower19(createMarkovMatrixFromPairs(sol20._1,sol20._2)).head.last)
...
//  matrixToPower10(createMarkovMatrixFromPairs(current,patientZero)).head.last) }
...
//println(solution)
/*
current Set(Set(2, 6), Set(1, 6), Set(3, 7), Set(1, 5), Set(2, 7), Set(4, 7), Set(0, 4), Set(1, 7), Set(1, 4), Set(4, 6), Set(0, 3), Set(0, 6), Set(0, 7), Set(3, 5))
patientZero 3
res 0.6999667649480299
Some((Set(Set(2, 6), Set(1, 6), Set(3, 7), Set(1, 5), Set(2, 7), Set(4, 7), Set(0, 4), Set(1, 7), Set(1, 4), Set(4, 6), Set(0, 3), Set(0, 6), Set(0, 7), Set(3, 5)),3))

Swapping 3 and 0.
Some((Set(Set(2, 6), Set(1, 6), Set(0, 7), Set(1, 5), Set(2, 7), Set(4, 7), Set(3, 4), Set(1, 7), Set(1, 4), Set(4, 6), Set(3, 0), Set(3, 6), Set(3, 7), Set(0, 5)),0))
