...
*/
  ...
  val numDigitsOfN = 200                          //> numDigitsOfN  : Int = 200
  ...
  def representPairs(n: BigInt): List[(Int, Int)] = n match {
    ...
    case _ =>    (0, maxPowerOf3Index(n)) :: representPairs(n - BigInt(3).pow(maxPowerOf3Index(n)))
  }
  ...
  val f = Stream.from(0).par.map { x =>
    ...
    if (x % 10000 == 0) {
      ...
      */
      ...
      def validateSum(r: List[(Int,Int)]) = r.map(x => BigInt(2).pow(x._1) * BigInt(3).pow(x._2)).sum == rando
      assert(validateSum(r), "sum")
      println("represnent rando " + r)
